...
// create DOM canvas
var canvas = document.createElement("canvas");//canvas is a container for any grphics
var ctx    = canvas.getContext('2d');//getContext will return the object which has methods and properties for drawing in the canvas
canvas.width  = window.innerWidth;//innerWidth returns the inner width of the window content area
canvas.height = window.innerHeight;

document.body.appendChild(canvas);// put this canvas into the body

function rand(min, max, interval) {
  if(interval === undefined) interval = 1;
  return Math.round((Math.floor(Math.random() * (max - min + 1)) + min) / interval) *interval;
}//Math.round is make the number to the nearest integer
...
var player = (function() {
  ...
  return {
    ...
    resurrect: function() {
      this.moveTo(100 , canvas.height/2);//this mean resurrect, and .moveTo(x,y). Question: in the screen, x for the left? y for the top?
      dead = false;
      this.draw();
    },
...
var blocks = (function() {
  ...
  var blocks = [],
      ...
      start = {
        n : 10,//
        x1 : 210,//move left in the canvas?
        x2 : 700,// move right in the canvas?
        h_min : 15,// storke min height
        h_max : 100,//storke max height
        speed_min : 0.5,
        speed_max : 5,
        direction : ['up','down']
      };
  ...
  return {
    ...
    nextLevel: function() {
      ...
      var n = Math.ceil(start.n + (level*level_factor));//math.ceil to round a decimal to an upward integer
      //console.log(n);
    ...
    draw: function(b) {
      if(player.isDead()) ctx.fillStyle = "#b90e41";
      else ctx.fillStyle = "#edce48";
    ...
    createXBlocks: function(n) {
      for(i=0;i<n;++i) {
        blocks.push(new Block(randIndex(start.direction)));//add more stroke
      }
    },
    ...
    moveAll: function() {
      ...
      for(i=0;i < len;++i) {
        ...
        //when the players hit the storke
...
function updateCtrl() {
  ...
  // apply some friction to y velocity (y seed).
  ctrl.velY *= ctrl.friction;
  ctrl.y += ctrl.velY;

  // apply some friction to x velocity (x speed).
...
// blocks of text
function draw() {
  ctx.clearRect(0, 0, canvas.width, canvas.height);//clear the specific px in a given object
  ...
  if(player.isDead()) {
    ctx.fillStyle = "#b90e41";
    ctx.font="14px Verdana";
    ctx.fillText("Game over!",10,50);
    ctx.fillText("Press [SPACE]",10,70);
  }
  else {
    ctx.fillStyle = "#edce48";
    ctx.font="14px Verdana";
